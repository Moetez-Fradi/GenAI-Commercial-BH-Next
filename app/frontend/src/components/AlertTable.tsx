...
export default function AlertTable({ title, alerts, onUpdateAlert }: AlertTableProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b bg-gradient-to-r from-red-500 to-orange-500 text-white">
        <h3 className="font-semibold text-lg">{title}</h3>
        <div className="flex items-center gap-2">
          {filtered && (
            <button onClick={clearFilters} className="text-xs px-3 py-1 rounded bg-white/20 hover:bg-white/30 transition">
              Clear
            </button>
          )}
        ...
        <div className="px-5 py-4 border-b bg-gray-50">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex flex-col">
              ...
              <select className="text-sm border rounded px-2 py-1" value={sortDir} onChange={(e) => setSortDir(e.target.value)}>
            ...
            <div className="flex flex-col">
              ...
              <select className="text-sm border rounded px-2 py-1" value={alertType} onChange={(e) => setAlertType(e.target.value)}>
            ...
            <div className="flex flex-col">
              ...
              <select className="text-sm border rounded px-2 py-1" value={product} onChange={(e) => setProduct(e.target.value)}>
                <option value="">All Products</option>
                {products.map(prod => <option key={prod} value={prod}>{prod}</option>)}
              </select>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <button onClick={() => setShowFilters(false)} className="px-3 py-1 text-sm rounded border bg-white hover:bg-gray-100 flex items-center gap-1">
                <X size={14} /> Close
              </button>
              <button onClick={applyFilters} disabled={loadingFilter} className="px-3 py-1 text-sm rounded bg-red-500 text-white hover:bg-red-600 disabled:opacity-60">
        ...
        <table className="w-full table-fixed text-left">
          ...
          <tbody>
            {rows.map((alert) => {
              ...
              return (
                <tr key={alert.ref_personne} className="border-b hover:bg-gray-50 transition cursor-pointer" onClick={() => setSelected(alert)}>
  ...
  <div className="flex items-center gap-2">
    ...
    <div className="min-w-0 flex-1">
      ...
      {clientDetails.type === 'moral' && clientDetails.estimatedBudget && (
        <div className="text-xs text-gray-500">Budget: {Math.round(Number(clientDetails.estimatedBudget))}</div>
      )}
    </div>
  </div>
</td>
                  ...
                  <td className="p-3 align-top">
                    {alert.days_until_expiry !== undefined ? (
                      <span className={`font-medium ${
                        ...
                        alert.days_until_expiry <= 30 ? 'text-orange-600' : 'text-green-600'
                      }`}>
            ...
            {!rows.length && (
              <tr>
                <td className="p-6 text-center text-gray-400" colSpan={7}>No alerts found</td>
              </tr>
            )}
          </tbody>
        </table>
