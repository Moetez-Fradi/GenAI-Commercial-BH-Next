...
export default function MessageComposer({ client, onClose, onSent, initialMessage }: Props) {
  ...
  const buildMessages = (c: Client) => {
    ...
    const who = anyC.type === "physique" ? `customer ${anyC.name ?? ref}` : `company ${anyC.raison_sociale ?? ref}`
  ...
  const extractPhone = (c: any): string | null => {
    const candidates = [c.phone, c.phone_number, c.phoneNumber, c.telephone, c.mobile, c.tel, c.contact_phone]
